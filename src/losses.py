...
class ComplexCompressedMSELoss(torch.nn.Module):
    ...
    def forward(self, enhanced: torch.tensor, clean: torch.tensor, w=None):
        enhanced_mag = torch.max(torch.abs(enhanced), 1e-8*torch.ones(enhanced.shape).to(enhanced.device())))
        clean_mag = torch.max(torch.abs(clean), 1e-8*torch.ones(clean.shape.to(clean.device())))
        ...
        base_loss = (1 - self.l) * mag_compressed_loss + self.l * phasor_loss
